...
def lobby():
    if 'user_id' in session:
        ...
        if user:
            ...
            return render_template('lobby.html', user=lobbyInfo['user'], games=lobbyInfo['games'],
                                   game_types=lobbyInfo['game_types'])
    return redirect('/login-registration')
    ...
    if 'user_id' in session:
        ...
        if user:
            ...
            if game and game.game_status != 0:
                ...
                return render_template('card-table.html', user=gameInfo['user'], game=gameInfo['game'],
                                       players=gameInfo['players'], cards=gameInfo['cards'])
            else:
                return redirect('/lobby')
    return redirect('/login-registration')
